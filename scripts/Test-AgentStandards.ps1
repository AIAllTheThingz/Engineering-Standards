...
if ($bashAgents) {
    ...
    $bashRequiredPatterns = @(
        ...
        @{ Pattern='Unverified local runs MUST record hosted execution as `NotRun`'; Message='Bash standard distinguishes local and hosted evidence.' },
        @{ Pattern='only a downloaded, identity-verified GitHub artifact may support a hosted `Passed` or `Failed` claim'; Message='Bash standard permits only verified hosted outcomes.' },
        @{ Pattern='Such a record MUST use `GitHubArtifact` and include run ID, run attempt, branch, artifact ID, artifact name, and artifact SHA-256'; Message='Bash standard requires concrete hosted artifact metadata.' },
        @{ Pattern='A failed required hosted check MUST retain overall `Failed`; local completion MUST NOT claim overall `Passed`'; Message='Bash standard preserves hosted failure and local completion status.' },
