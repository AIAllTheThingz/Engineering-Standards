...
if ($workflows.ContainsKey($prReusable)) {
    ...
    if ($prText -notmatch 'github\.workflow_sha' -or $prText -notmatch 'persist-credentials:\s*false') { $results.Add((New-ValidationResult -Status Failed -Message 'PR governance workflow must check out only immutable trusted workflow content without credentials.' -Path $prReusable)) }
