...
Describe 'Governed Bash project support' {
    ...
    It 'keeps checked-in local completion evidence valid and honestly NotRun' {
        ...
        $allowedAfterValidation = @(
            ...
            'docs/releases/unreleased-consolidation.md',
            ...
            'tests/scripts/DependabotConfiguration.Tests.ps1',
...
"@
    ...
    It 'detects representative workflow-control mutations' -ForEach @(
        ...
        @{ Name='completion copies unnormalized evidence'; Mutate={ param($text) $text.Replace("if: always() && steps.bootstrap.outcome == 'success' && steps.regression.outcome == 'success' && steps.normalization.outcome == 'success'",'if: always()') }; Expected='unsafe-evidence-upload' },
        @{ Name='upload ignores evidence safety gates'; Mutate={ param($text) $text.Replace("if: always() && steps.bootstrap.outcome == 'success' && steps.regression.outcome == 'success' && steps.normalization.outcome == 'success' && steps.completion.outcome == 'success' && steps.evidence.outcome == 'success'",'if: always()') }; Expected='unsafe-evidence-upload' },
