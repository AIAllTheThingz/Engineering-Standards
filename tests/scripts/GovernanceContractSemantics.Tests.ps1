BeforeAll {
    ...
    function Invoke-Semantics([hashtable]$Manifest, [hashtable]$Config, [string]$ExpectedRepository = 'AIAllTheThingz/Engineering-Standards', [string]$ExpectedStandardsRepository = 'AIAllTheThingz/Engineering-Standards', [string]$OwnerType = 'User', [string]$ExpectedSha = '5450976a4cf02661854df24f4427082242821edd', [string]$Interface = '1.0.0', [string]$Profile = 'standards-maintainer', [string]$Check = 'Governance / Governance validation', [string]$Root = $script:root) {
...
Describe 'Governance contract semantic validation' {
    ...
    It 'binds central-reference repository and commit identity to trusted workflow context' -ForEach @(
        @{ Name='repository mismatch'; SourceRepository='Untrusted/Standards'; SourceSha='5450976a4cf02661854df24f4427082242821edd'; GovernanceSha='5450976a4cf02661854df24f4427082242821edd'; Pattern='trusted standards repository' },
        ...
        @{ Name='governance SHA disagreement'; SourceRepository='AIAllTheThingz/Engineering-Standards'; SourceSha=('b' * 40); GovernanceSha='5450976a4cf02661854df24f4427082242821edd'; Pattern='declared governance commit SHA' }
    ) {
        $manifest = Copy-ContractObject $script:manifest
        $config = Copy-ContractObject $script:config
        ...
        $expectedSha = if ($Name -eq 'trusted SHA mismatch') { '5450976a4cf02661854df24f4427082242821edd' } else { $SourceSha }
    ...
    It 'enforces the central-reference standards-consumption field contract without schema validation' -ForEach @(
        ...
        $manifest.standardsConsumption = @{
            ...
            sourceCommitSha = '5450976a4cf02661854df24f4427082242821edd'
        }
