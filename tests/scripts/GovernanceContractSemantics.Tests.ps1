BeforeAll {
    ...
    function Invoke-Semantics([hashtable]$Manifest, [hashtable]$Config, [string]$ExpectedRepository = 'AIAllTheThingz/Engineering-Standards', [string]$ExpectedStandardsRepository = 'AIAllTheThingz/Engineering-Standards', [string]$OwnerType = 'User', [string]$ExpectedSha = 'a56b26b1c07a01f7c0b83fb013d0c15f1d344d5b', [string]$Interface = '1.0.0', [string]$Profile = 'standards-maintainer', [string]$Check = 'Governance / Governance validation', [string]$Root = $script:root) {
...
Describe 'Governance contract semantic validation' {
    ...
    It 'binds central-reference repository and commit identity to trusted workflow context' -ForEach @(
        @{ Name='repository mismatch'; SourceRepository='Untrusted/Standards'; SourceSha='a56b26b1c07a01f7c0b83fb013d0c15f1d344d5b'; GovernanceSha='a56b26b1c07a01f7c0b83fb013d0c15f1d344d5b'; Pattern='trusted standards repository' },
        ...
        @{ Name='governance SHA disagreement'; SourceRepository='AIAllTheThingz/Engineering-Standards'; SourceSha=('b' * 40); GovernanceSha='a56b26b1c07a01f7c0b83fb013d0c15f1d344d5b'; Pattern='declared governance commit SHA' }
    ) {
        $manifest = Copy-ContractObject $script:manifest
        $config = Copy-ContractObject $script:config
        ...
        $expectedSha = if ($Name -eq 'trusted SHA mismatch') { 'a56b26b1c07a01f7c0b83fb013d0c15f1d344d5b' } else { $SourceSha }
    ...
    It 'enforces the central-reference standards-consumption field contract without schema validation' -ForEach @(
        ...
        $manifest.standardsConsumption = @{
            ...
            sourceCommitSha = 'a56b26b1c07a01f7c0b83fb013d0c15f1d344d5b'
        }
