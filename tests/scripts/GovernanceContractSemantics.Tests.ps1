BeforeAll {
    ...
    function Invoke-Semantics([hashtable]$Manifest, [hashtable]$Config, [string]$ExpectedRepository = 'AIAllTheThingz/Engineering-Standards', [string]$ExpectedStandardsRepository = 'AIAllTheThingz/Engineering-Standards', [string]$OwnerType = 'User', [string]$ExpectedSha = '0e2b527b158bc6e2c6b8995786565e7a866c2d20', [string]$Interface = '1.0.0', [string]$Profile = 'standards-maintainer', [string]$Check = 'Governance / Governance validation', [string]$Root = $script:root) {
...
Describe 'Governance contract semantic validation' {
    ...
    It 'binds central-reference repository and commit identity to trusted workflow context' -ForEach @(
        @{ Name='repository mismatch'; SourceRepository='Untrusted/Standards'; SourceSha='0e2b527b158bc6e2c6b8995786565e7a866c2d20'; GovernanceSha='0e2b527b158bc6e2c6b8995786565e7a866c2d20'; Pattern='trusted standards repository' },
        ...
        @{ Name='governance SHA disagreement'; SourceRepository='AIAllTheThingz/Engineering-Standards'; SourceSha=('b' * 40); GovernanceSha='0e2b527b158bc6e2c6b8995786565e7a866c2d20'; Pattern='declared governance commit SHA' }
    ) {
        $manifest = Copy-ContractObject $script:manifest
        $config = Copy-ContractObject $script:config
        ...
        $expectedSha = if ($Name -eq 'trusted SHA mismatch') { '0e2b527b158bc6e2c6b8995786565e7a866c2d20' } else { $SourceSha }
    ...
    It 'enforces the central-reference standards-consumption field contract without schema validation' -ForEach @(
        ...
        $manifest.standardsConsumption = @{
            ...
            sourceCommitSha = '0e2b527b158bc6e2c6b8995786565e7a866c2d20'
        }
