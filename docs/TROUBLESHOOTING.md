...
For a nonzero trusted Codex subprocess exit, the controlled behavior evaluator records only a canonical category, numeric exit code, and governed retry decision. The categories are `AuthenticationFailed`, `AuthorizationFailed`, `QuotaExceeded`, `RateLimited`, `ModelUnavailable`, `ConfigurationError`, `TransportFailure`, `TransportTimeout`, `ProviderError`, and `UnknownProviderFailure`. One bounded retry is permitted only for `ModelUnavailable`, `TransportTimeout`, `TransportFailure`, and `ProviderError`; authentication, authorization, quota, rate-limit, configuration, ambiguous, and unknown outcomes remain non-retryable. `TransportFailure` and `ProviderError` are evidence-schema `1.1.0` additions; schema `1.0.0` retains its original retry-reason enum. A missing approved credential or a preflight process launch/setup failure is a `PreflightUnavailable` one-attempt block, not a simulated provider retry. Once the process has started, retryable transport failures continue through the governed per-sample retry path. A category is diagnostic evidence, not proof of a provider root cause; `UnknownProviderFailure` is the required fail-closed fallback. Do not add raw stdout, stderr, URLs, requests, responses, or credentials to workflow artifacts while investigating a blocked result. Evidence schema `1.1.0` requires behavior reports to declare `executionContext: Local` and `githubHostedExecution.status: NotRun`: process environment variables cannot authenticate GitHub provenance. Evidence schema `1.2.0` requires the persistence-boundary hash to match the trusted evaluator boundary, while `1.3.0` additionally requires the complete squash-safe evaluated-input hash. Only a separately verified workflow artifact may establish hosted execution. Existing schema `1.0.0` and legacy `1.2.0` records remain accepted without newer fields; they must not add provenance or hash fields retroactively or invent GitHub run metadata.
