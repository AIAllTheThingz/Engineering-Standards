...
For the downstream profile, each required document must contain at least 100 words and three headings (levels 1–3, allowing zero to three leading spaces), with meaningful section bodies. Heading titles must contain non-whitespace parsed text; empty HTML tags and whitespace-only entities or code spans do not count as section names. Formatting, link labels, image alternative text, and code literals with text remain valid. HTML comments do not contribute words, headings, or section bodies, and fenced code headings do not count as document sections. Empty files, shallow stubs, and unresolved placeholders fail. Central authoritative documents retain their stricter depth and required-concept checks. The repository-wide empty-heading scan excludes unfilled pull-request and Markdown issue templates; explicitly required documents still receive completeness checks.
