...
For the downstream profile, each required document must contain at least 100 words and three headings (levels 1–3, allowing zero to three leading spaces), with meaningful section bodies. Both ATX (`#`) and Setext (underline-style) headings count. Heading titles must contain non-whitespace parsed text after ignoring Unicode format and control characters; empty HTML tags and whitespace-only entities or code spans do not count as section names. Formatting, link labels, Markdown image alternative text, and code literals with text remain valid. Word counts and section bodies use parsed text: invisible entities, empty markup, HTML attributes, script/style content, YAML front matter, and HTML comments do not supply content. Visible code literals remain content, and code-block headings, including fences nested in lists, do not count as document sections. Empty files, shallow stubs, and unresolved placeholders fail. Central authoritative documents retain their stricter depth and required-concept checks. The repository-wide empty-heading scan excludes unfilled pull-request and Markdown issue templates; explicitly required documents still receive completeness checks.

Legacy `1.0.0` and `1.1.0` configurations may omit `workflowProfile`. Their documentation profile comes from the canonical root `project-manifest.json`, matching aggregate profile selection: only the `AIAllTheThingz/Engineering-Standards` repository with project type `governance` receives the central maintainer requirements; other repositories use their configured downstream documentation paths. Optional legacy profile hints and alternate manifest paths do not override that identity.
