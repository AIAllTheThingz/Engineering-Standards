...
This file defines the reusable AI-agent instruction contract for WebFrontend work. It is not a standalone policy; it inherits `agents/AGENTS_Base.md` and adds technology-specific requirements.

## Scope

Browser-facing applications, static sites, client bundles, and frontend build systems.
...
- Identify runtime, package manager, lockfile, lint, tests, build, accessibility tooling, auth token handling, CSP, and browser support.
...
- Use lockfile-enforced installs, linting, formatting, unit tests, browser tests where useful, build validation, accessibility checks, safe HTML handling, XSS prevention, CSRF awareness, no client-side secrets, source-map review, and performance budgets.

## Required Validation And Testing

- Package-manager install with lockfile.
- Lint, format, unit tests, build, dependency audit, and accessibility checks when configured.
...
- A token is stored in an httpOnly server-managed cookie, not localStorage.
