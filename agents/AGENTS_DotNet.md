...
This file defines the reusable AI-agent instruction contract for DotNet work. It is not a standalone policy; it inherits `agents/AGENTS_Base.md` and adds technology-specific requirements.

## Scope

ASP.NET Core, worker services, libraries, CLIs, APIs, and .NET-hosted services.
...
- Locate solution, target frameworks, nullable settings, package lockfiles, configuration, secrets usage, authentication, migrations, and tests.
...
- Use supported .NET versions, nullable reference types, dependency injection, options validation, structured logging, cancellation tokens, timeouts, health checks, package auditing, lockfiles, and warnings as errors.
- Protect secrets through configuration providers and never commit local settings with credentials.
- Use EF Core migrations safely with rollback notes.

## Required Validation And Testing

- `dotnet restore --locked-mode` where lockfiles exist.
- `dotnet format --verify-no-changes`.
- `dotnet build -warnaserror`.
- `dotnet test` and integration tests where configured.
...
- An API endpoint validates model state and authorization policy before calling services.
