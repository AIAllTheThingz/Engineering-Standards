...
This file defines the reusable AI-agent instruction contract for PowerShell work. It is not a standalone policy; it inherits `agents/AGENTS_Base.md` and adds technology-specific requirements.

## Scope

PowerShell scripts, modules, scheduled jobs, administration automation, REST clients, PowerCLI, and remote-management tooling.
...
- Identify PowerShell version requirements.
- Check whether Windows PowerShell 5.1 compatibility is required.
- Locate module manifests, PSScriptAnalyzer settings, Pester tests, credential flows, remoting usage, and destructive operations.
- Classify operations into discovery, validation, simulation, and execution phases.
...
- Use `SupportsShouldProcess` for changes and explain `-WhatIf` versus broader `-DryRun` simulation.
- Use strict mode and `$ErrorActionPreference = 'Stop'` at entry points.
- Avoid aliases and compressed one-line scripts.
- Handle CurrentUser, Prompt, and CyberArk CCP credential modes without plaintext secrets.
- Do not enable WinRM automatically.
- Use idempotency, retries, timeouts, rollback, and structured output where appropriate.
- Keep Authenticode signing compatible and avoid self-modifying signed scripts.

## Required Validation And Testing

- Parser validation.
- PSScriptAnalyzer when available.
- Pester tests for success, failure, WhatIf, path validation, credential priority, and nonzero exit behavior.
- Manual validation for remote administration when no lab is available.
...
- `Remove-ExampleItem -WhatIf` shows intended change without mutating state.
- `Invoke-ExampleDiscovery -DryRun` validates inputs and reports planned API calls without execution.
