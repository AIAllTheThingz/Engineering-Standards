Describe 'Python review home-lab demo' {
    BeforeAll {
        ...
        $script:skillPath = Join-Path $script:demoRoot '.agents/skills/python-review/SKILL.md'
        $script:samplePath = Join-Path $script:demoRoot 'samples/unsafe_maintenance.py'
    }

    It 'keeps the skill outside production discovery' {
        Test-Path -LiteralPath (Join-Path $script:standardsRoot '.agents/skills/python-review/SKILL.md') | Should -BeFalse
    ...
    It 'declares demo-only read-only boundaries' {
        ...
        $skill | Should -Match 'Do not import, execute'
    }
    ...
    It 'provides exactly nine unique prompt categories' {
        ...
        @($cases | Where-Object skillName -cne 'python-review').Count | Should -Be 0
    ...
    It 'keeps the unsafe Python sample synthetic and byte-matched to its diff' {
        $sample = Get-Content -LiteralPath $script:samplePath -Raw
        $sample | Should -Match 'https://example\.invalid/'
        $sample | Should -Match 'shell=True'
        $sample | Should -Match 'shutil\.rmtree'
    ...
    It 'labels illustrative output honestly' {
        ...
        foreach ($id in 1..6 | ForEach-Object { 'PYR-{0:D3}' -f $_ }) { $review | Should -Match $id }
    }
}
