Describe 'Bash review home-lab demo' {
    BeforeAll {
        ...
        $script:skillPath = Join-Path $script:demoRoot '.agents/skills/bash-review/SKILL.md'
        $script:samplePath = Join-Path $script:demoRoot 'samples/unsafe-maintenance.sh'
    ...
    It 'keeps the skill outside production discovery' {
        Test-Path -LiteralPath (Join-Path $script:standardsRoot '.agents/skills/bash-review/SKILL.md') | Should -BeFalse
    ...
    It 'declares demo-only read-only boundaries' {
        ...
        $skill | Should -Match 'Do not source, execute'
    }
    It 'provides exactly nine unique prompt categories' {
        ...
        @($cases | Where-Object skillName -cne 'bash-review').Count | Should -Be 0
    ...
    It 'keeps the unsafe Bash sample synthetic and matched to its diff' {
        $sample = Get-Content -LiteralPath $script:samplePath -Raw
        $sample | Should -Match 'https://example\.invalid/'
        $sample | Should -Match 'rm -rf \$item'
        $sample | Should -Match 'Authentication material'
    ...
    It 'labels illustrative output honestly' {
        ...
        foreach ($id in 1..6 | ForEach-Object { 'BSR-{0:D3}' -f $_ }) { $review | Should -Match $id }
    }
}
